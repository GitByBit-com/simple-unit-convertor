...
import {ConversionResult, Measure, Unit} from './types';

function getMeasure(unit: Unit): Measure | null {
...
function findUnitInMeasure(measure: Measure, unit: Unit) {
...
export function convert(value: number, unit: Unit): ConversionResult {
  return {
    value,
    unit,
    to: (targetUnit: Unit) => {
